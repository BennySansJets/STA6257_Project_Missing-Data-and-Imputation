...
As others have noted,[@mhth19] providing sufficient auxiliary variables can alleviate power degradation from missing data. They determined that standard error increased when the fraction of missing information (FMI) increased, while FMI decreased when auxiliary variables increased. 
...
It is essential to have quality datasets to avoid corrupted machine learning models. A derivative of MI is the Multivariate Imputation by Chained Equation (MICE) algorithm which assumes data is missing at random (MAR). The process uses multiple imputation methods to provide better results by considering the uncertainty of missing data. Others[@kh20] provide an alternative to MICE. 
...
For our analysis, we decided to use the R packages for predictive means matching, classification and regression trees, and lasso linear regression. Predictive means matching(PMM) is a hot deck method, which pools complete cases which are similar to the case we are trying to impute, and randomly chooses values from the pool of prospective donors to be imputed for the missing data. Classification and regression trees(CART) looks for predictors and cutoff points in the data, and splits data into trees along these points. Missing data is replaced very similarly to the PMM method, but data is chosen according to the tree of splitting points rather than regression. Lasso linear regression fits a linear regression using the missing columns as dependent and complete columns as predictors. Then a Bayesian linear model is defined, and imputations are drawn from the posterior predictive distribution.  


### Analysis and Results

Prior to implementing the MICE method, we ran a missing map to get a count of which columns contain missing data and how many data points are missing. Below are the MICE code and results, and figure.
Code:
...
```
```{r, warning=FALSE, echo=T, message=FALSE}
# Calculate pattern of missing data (Unsure how to access the data from the github so it works on the web)
#Chronic_Conditions_STA6707_07132023 <- Chronic_Conditions_Sta6707_07132023 %>%
 # +select(Prvlnc, Tot_Mdcr_Stdzd_Pymt_PC, Tot_Mdcr_Pymt_PC)
#md.pattern(Chronic_Conditions_STA6707_07132023)
...
Figure 1: Missing Map 
 

Following the Missing map, we ran the MICE package in R. Below are the MICE code and results.
```{r, warning=FALSE, echo=T, message=FALSE}
# Imputes missing data using the three selected methods 
#mice_imputed <- data.frame ( 
 # +imputed_pmm = complete(mice(Chronic_Conditions_Sta6707_07132023_numeric, method = "pmm"))$Prclnc,
  #+imputed_cart = complete(mice(Chronic_Conditions_Sta6707_07132023_numeric, method = "cart"))$Prclnc,
  #+imputed_lasso = complete(mice(Chronic_Conditions_Sta6707_07132023_numeric, method = "lasso.norm"))$Prclnc,
  #+)
...
After running the MICE package, we ran a MICE plot to gain a better visual of our missing data. Below are the code, results, and figure. 
```{r, warning=FALSE, echo=T, message=FALSE}
#mice_plot < - aggr(Chronic_Conditions_Sta6707_07132023.mis, col=c('navyblue', 'yellow' ),
 #                  numbers=TRUE, sortVars=TRUE,
  #                 labels=names(Chronic_Conditions_Sta6707_07132023.mis), cex.axis=.7,
   #                gap=3, ylab=c("Missing Data", "Pattern"))
#imputed_Data < - mice(Chronic_Conditions_Sta6707_07132023.mis, m=5, maxit=50, method = 'pmm', seed=500)
...
Figure 2: Missing data percentages per variable
...
```{r, warning=FALSE, echo=T, message=FALSE}
#Chronic_Conditions_Sta6707_07132023.mis <- prodNA(Chronic_Conditions_Sta6707_07132023, noNA = 0.1)
#md.pattern(Chronic_Conditions_Sta6707_07132023.mis)
#summary(Chronic_Conditions_Sta6707_07132023.mis)
```
...
Figure 3: Table of missing values present in each variable in dataset
